  ...
  return (
    <div className="mx-auto w-full max-w-xl">
      ...
      <div className="flex flex-col gap-4">
        ...
        {/* Stack */}
        <div>
          <label className="text-sm text-gray-500 block mb-1">Effective Stack (bb)</label>
          <input
            type="number"
            step="0.5"
            min="1"
        ...
        {/* Acting Position */}
        <div>
          <label className="text-sm text-gray-500 block mb-1">Acting Position</label>
          <select
        ...
        {/* History */}
        <div>
          <label className="text-sm text-gray-500 block mb-1">Previous Actions</label>
          {history.map((entry, i) => (
            <div key={i} className="flex gap-2 mb-2">
              <select
                value={entry.position}
                onChange={(e) => updateHistoryEntry(i, 'position', e.target.value)}
                className="flex-1 rounded-lg border border-gray-200 bg-white px-2 py-1 text-sm text-gray-900"
              >
                {positions.map((p) => (
                  <option key={p} value={p}>{p}</option>
                ))}
        ...
        <div>
          ...
          <textarea
            ...
            className="h-20 w-full resize-none rounded-lg border border-gray-200 bg-white px-3 py-2 text-gray-900"
          />
        </div>
